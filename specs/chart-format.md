...
8. 같은 곡(songId)에 여러 채보(chartId, 난이도별)가 속하는 곡-채보 2계층 구조를 가진다. 곡 레벨에 음원 참조(URL)와 음원 재생 오프셋(ms)을 둔다. 키음 연습곡([practice-song-content](practice-song-content.md))은 이 곡 레벨 음원 참조가 BGM 트랙 대신 키음 샘플을 가리킨다(구조는 동일).
