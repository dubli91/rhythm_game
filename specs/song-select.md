...
내장 곡 카탈로그([song-library](song-library.md))를 탐색하고,
...
1. 곡 목록을 표시한다: 제목, 아티스트, 장르, BPM(변속 곡은 min~max).
...
9. 곡 결정 시 채보·음원을 로드하고(fetch) 로딩 표시 후 플레이 화면으로 전환한다. 로드 실패 시 곡 선택으로 복귀하며 원인을 표시한다.
10. 곡 선택 화면에서 연습 세션([practice-mode](practice-mode.md))과 설정(키 컨피그·오프셋·볼륨) 화면으로 이동할 수 있다.
...
- 전환: 플레이 화면, [practice-mode](practice-mode.md), 설정
