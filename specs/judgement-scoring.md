...
10. 판정별 카운트(PGREAT/GREAT/GOOD/BAD/POOR, 공POOR 별도)를 집계한다. BP(BAD + 미스 POOR + 공POOR + CN 도중 이탈 합계)를 계산한다 — 공POOR·CN 도중 이탈은 등급 카운트에는 넣지 않지만 BP에는 합산한다.
...
13. 판정 분포(δ 히스토그램)를 세션 중 수집해 결과 화면·연습 모드에서 활용. 누산기는 스코어러에 두어 곡 플레이와 연습(루프마다 스코어러 1개)이 단일 소스를 공유하고, δ 기반 판정만(PGREAT 포함, 미스POOR·공POOR·CN 꼬리 판정 제외)을 부호 있는 δ로 담는다 — ±75ms를 10ms 폭 15개 버킷으로 나누고 범위 밖 값은 양 끝 버킷에 클램프한다. 결과 화면에서는 카운트 그리드 아래에 8단계 텍스트 스파크라인(예: `δ −75ms ▁▃█▃▁ +75ms`, 0이 아닌 버킷은 최소 ▁, 유효 히트가 없으면 빈칸)으로 그리고, 연습 HUD 누적 통계도 같은 버킷 정의를 쓴다([practice-mode](practice-mode.md) SHOULD 13). 표시 전용이며 기록([results-records](results-records.md))에는 저장하지 않는다.
